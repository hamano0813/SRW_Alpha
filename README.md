...
从 [Releases](https://github.com/hamano0813/SRW_Alpha/releases) 页面可以下载到两种压缩包：

- **完整安装包**（如 `SRW_Alpha_v0.x.x.zip`）：包含修改器本体、运行环境和所有依赖，解压后运行安装程序即可使用（安装过程需要联网，可能需要等待几分钟）
- **升级包**（如 `v0.x.x.zip`）：仅包含更新文件，适合已有旧版本的用户，下载后解压并覆盖到程序目录即可
