...
内嵌自绘 QLineEdit，完成表格内文本编辑。
首列/中间列/末列使用对应的圆角方向编辑器。
...
from .editor_lineedit import FirstColLineEdit, MidColLineEdit, LastColLineEdit


class LineTextFirstWidget(DataWidget):
    """首列单行文本编辑器 — 文本即原始值
...
class LineTextMidWidget(DataWidget):
    """中间列单行文本编辑器 — 文本即原始值

    内嵌 MidColLineEdit（纯 QLineEdit 自绘，无 QSS 干扰），
    四角直角，与表格中间列视觉一致。
    """
    ...
    def __init__(self, parent=None):
        ...
        self._line_edit = MidColLineEdit()
        self._line_edit.setSizePolicy(
            QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding
        )
        # 微调文本位置 + 透明背景（背景由 delegate 层绘制，编辑器不额外遮挡）
...
class LineTextLastWidget(DataWidget):
    """末列单行文本编辑器 — 文本即原始值

    内嵌 LastColLineEdit（纯 QLineEdit 自绘，无 QSS 干扰），
    左直右圆，与表格末列视觉一致。
    """
    ...
    def __init__(self, parent=None):
        ...
        self._line_edit = LastColLineEdit()
        self._line_edit.setSizePolicy(
            QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding
        )
        # 微调文本位置 + 透明背景（背景由 delegate 层绘制，编辑器不额外遮挡）
